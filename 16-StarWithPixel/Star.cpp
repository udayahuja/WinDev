...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (iMsg) {
		case WM_CREATE:
			...
			//penStar = CreatePen(PS_SOLID, 3, RGB(255, 0, 0));
		...
		case WM_PAINT:
			...
			//SelectObject(hdc, penStar);

			for (int i = 0; i < 700; i++) {
				//for(int i=0;i<3;i++)
					//for(int j=0;j<3;j++)
						SetPixel(hdc, rc.left + rand()%(rc.right - rc.left)  ,rc.top + rand()%(rc.bottom - rc.top) , RGB(0,255,0));

			}
			SetBkColor(hdc, RGB(0,0,0));
			SetTextColor(hdc, RGB(255,0,0));
			TextOut(hdc, (rc.right - rc.left)/3, rc.top + 5, TEXT("Aries Star Constilaion"), 22);
			EndPaint(hWnd, &ps);
			break;
		case WM_SIZE:
			GetClientRect(hWnd, &rc);
